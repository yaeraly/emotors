...
export class OperationsController {
  ...
  @Roles(Role.OWNER, Role.CEO, Role.FRANCHISE_OWNER, Role.MANAGER, Role.SUPPLY_CHAIN_MANAGER, Role.WAREHOUSE_MANAGER)
  ...
  @Roles(Role.OWNER, Role.CEO, Role.WAREHOUSE_OPERATOR, Role.WAREHOUSE_MANAGER)
  releaseWarehouseOrder(@CurrentUser() user: AuthUser, @Param('id') id: string) {
    return this.service.releaseWarehouseOrder(user, id);
  }

  @Get('parts-requests')
  @Roles(Role.OWNER, Role.CEO, Role.FRANCHISE_OWNER, Role.MASTER, Role.WAREHOUSE_OPERATOR)
  ...
  @Get('returns')
  @Roles(Role.OWNER, Role.CEO, Role.FRANCHISE_OWNER, Role.MANAGER, Role.CASHIER)
  ...
  @Get('warranty/claims')
  @Roles(Role.OWNER, Role.CEO, Role.FRANCHISE_OWNER, Role.MASTER, Role.SUPPLY_CHAIN_MANAGER, Role.WAREHOUSE_MANAGER)
  ...
  @Get('alerts')
  @Roles(Role.OWNER, Role.CEO, Role.FRANCHISE_OWNER, Role.MANAGER, Role.MASTER, Role.WAREHOUSE_OPERATOR, Role.CASHIER, Role.SUPPLY_CHAIN_MANAGER, Role.WAREHOUSE_MANAGER, Role.FINANCE_MANAGER, Role.ACCOUNTANT)
  alerts(@CurrentUser() user: AuthUser) {
    return this.service.alerts(user);
  }

  @Post('alerts')
  @Roles(Role.OWNER, Role.CEO, Role.SUPPLY_CHAIN_MANAGER, Role.WAREHOUSE_MANAGER, Role.FINANCE_MANAGER)
  createAlert(@CurrentUser() user: AuthUser, @Body() dto: any) {
    return this.service.createAlert(user, dto);
  }

  @Get('operations/analytics')
  @Roles(Role.OWNER, Role.CEO, Role.FRANCHISE_OWNER, Role.SUPPLY_CHAIN_MANAGER, Role.FINANCE_MANAGER, Role.ACCOUNTANT)
