      ...
      void apiFetch<SaleProductOption[]>(`/sales/product-options?${params.toString()}`)
        .then((items) => {
          ...
          const ranked = rankedIds
            ...
            .filter((item): item is SaleProductOption => Boolean(item));
          setResults(ranked);
          setOpen(true);
          setHighlightedIndex(0);
          setError(ranked.length ? '' : t('sales.productSearch.noResults'));
        })
        .catch((err) => {
          setResults([]);
          setOpen(false);
  ...
  return (
    <div ref={containerRef} className="relative">
      <label className="block">
        <span className="text-sm font-semibold text-slate-700">{t('sales.productSearch.label')}</span>
        <input
          ref={searchInputRef}
          type="search"
          role="combobox"
      ...
      <p className="mt-2 text-xs text-slate-500">{t('sales.productSearch.hint')}</p>
