...
type EditableWholesaleRow = WholesaleRow & ReturnType<typeof createMarkupRowEditorState>;

function toEditableRow(product: WholesaleRow): EditableWholesaleRow {
  return {
    ...product,
...
export default function PricingWholesalePage() {
  ...
  function requestPreview(row: EditableWholesaleRow) {
    schedulePreview(row.id, {
      ...
      maximumWholesaleMarkupOverridePercent: row.maximumWholesaleMarkupOverridePercent,
    });
  }
  ...
  function rowValidationMessage(row: EditableWholesaleRow) {
