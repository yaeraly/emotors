...
function CashierBillsPageContent() {
  ...
  async function uploadReceipt(row: BillRow) {
    if (!receiptFile) {
      throw new Error(t('finance.cashierBills.receiptRequired'));
    }
    ...
    if (row.source === 'SUPPLIER_PAYMENT') {
      const orderId = selected?.relatedOrderId || row.relatedOrderId;
    ...
    <ProtectedShell>
      <div className="space-y-4 p-4 md:p-6">
          ...
          <div className="overflow-x-auto rounded-xl border border-slate-200 bg-white">
            <table className="min-w-full divide-y divide-slate-200 text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                {data.items.map((row) => {
                  const busy = row.executionStatus === 'COMPLETED' || row.executionStatus === 'RETURNED_TO_ACCOUNTANT' || row.executionStatus === 'CANCELLED';
                  return (
                    <tr key={`${row.source}-${row.id}`}>
                      ...
                      <td className="px-3 py-2">
                        <div className="flex flex-wrap items-center gap-1.5">
                          ...
                          {!busy && row.executionStatus !== 'IN_PROGRESS' ? (
                            <button
                              type="button"
                              disabled={saving}
                          ...
                          {!busy ? (
                            <button
                              type="button"
                              disabled={saving}
                              onClick={() => { setReasonModal({ mode: 'return', row }); setReason(RETURN_REASONS[0]); }}
                          ...
                          {!busy ? (
                            <button
                              type="button"
                              disabled={saving}
                              onClick={() => { setReasonModal({ mode: 'fail', row }); setReason(''); }}
        ...
        {selected ? (
          <div className="fixed inset-0 z-40 flex justify-end bg-slate-950/40" onClick={() => setSelected(null)}>
            ...
            >
              ...
              {selected.href ? (
                <Link href={selected.href} className="mt-4 inline-block text-sm font-semibold text-blue-600">
                  {t('finance.cashierBills.openSource')}
                </Link>
              ) : null}
            </aside>
          </div>
        ...
        {confirmModal && activeRow ? (
          <Modal title={t('finance.cashierBills.confirm')} onClose={() => setConfirmModal(null)}>
            ...
            <button
              type="button"
              disabled={saving || !paymentDate || !receiptFile}
