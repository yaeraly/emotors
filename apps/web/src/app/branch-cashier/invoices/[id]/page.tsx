            ...
            {invoice.workflowStatus !== 'PAID' ? (
              <form onSubmit={submitPayment} className="grid gap-4 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm md:grid-cols-2">
                ...
                <label className="block">
                  ...
                  <input value={amount} onChange={(e) => setAmount(e.target.value)} type="number" min="0.01" step="0.01" className="mt-2 w-full rounded-xl border border-slate-300 px-3 py-2" required />
                </label>
                <label className="block">
