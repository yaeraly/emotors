  ...
  return (
    <ProtectedShell>
      <section className="space-y-6">
        ...
        >
          <Input label={t('inventory.categoryCode')} value={form.code} onChange={(value) => setForm({ ...form, code: value })} />
          <Input label={t('inventory.categoryNameKy')} value={form.nameKy} onChange={(value) => setForm({ ...form, nameKy: value })} />
          <Input label={t('inventory.categoryNameRu')} value={form.nameRu} onChange={(value) => setForm({ ...form, nameRu: value })} />
          <Input label={t('inventory.categoryNameEn')} value={form.nameEn} onChange={(value) => setForm({ ...form, nameEn: value })} />
          <Input label={t('inventory.description')} value={form.description} onChange={(value) => setForm({ ...form, description: value })} />
                  ...
                  <tr key={category.id}>
                    ...
                    <td className="px-4 py-3">
                      ...
                      <div className="flex gap-2">
                        ...
                        <button onClick={() => void deleteCategory(category)} className="rounded-lg border border-red-200 px-3 py-2 text-xs font-semibold text-red-600" type="button">
                          {t('common.delete')}
                        </button>
                      </div>
                      ) : '—'}
...
function Input({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) {
  return (
    <label className="block">
      ...
      <input value={value} onChange={(event) => onChange(event.target.value)} required className="mt-2 w-full rounded-xl border border-slate-300 px-3 py-2" />
    </label>
  );
}
